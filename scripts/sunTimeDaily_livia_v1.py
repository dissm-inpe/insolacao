...
##-------Paths--------##
#basePath = '/'.join(['/dados','goes','goes19_produtos','rad_solar'])
#folderRef = basePath + '/refletancia_vis_bin'
#outPath   = '/home/maria.gava/sunshineTime/sunshineTime/output'
#mainPath  = basePath
basePath = '/home/maria.gava/desktop/Rad_solar/sunshineTime'
folderRef = basePath + '/bruto'
...
outputIMG = outPath +"/insol_diaria/"+str(year)+"/"+str(month)+"/S11167057_v1_"+inputdate+"0000.png"
pathBin = outPath +"/insol_diaria_bin/"+str(year)+"/"+str(month)+"/S11167051_v1_"+inputdate+"0000.bin"
#pathPGW = outPath +"/insol_diaria/"+str(year)+"/"+str(month)+"/S11167057_"+inputdate+"0000.pgw"

##-----Create list of files for the date------------##
filesRef = sorted(glob.glob(folderRef + "/" + str(year) + "/" + str(month) +
                           "/S11167029_" + inputdate + "????.ref"))
...
dayJ = RegularFunc_v1.calcDayJul(yearI, monthI, dayI)

##------Function to return global parameters----##
PI,convgr, Csolar0, Suvb0, Svisivel0, Sinfra0 = LibSUN_v1.Parametros()

##------Function to calculate astronomic parameters----##
delta, dtempo = LibSUN_v1.paramAstron(dayJ, PI, Csolar0, Suvb0, Svisivel0, Sinfra0)

##----Return time of sunrise and sunset-----------##
horaini, horafin = LibSUN_v1.StartEndSun(xx, yy, PI, convgr, delta, dtempo)
...
matTAux=np.full((rows, cols), np.nan)
dataAux=np.full((rows, cols), np.nan)
...
refX1 = np.fromfile(pathFileX1, np.uint16)
refX1 = refX1.reshape(1800, 1800)
refX1 = np.array(refX1, float)/10.

dataX1= LibSUN_v1.clearSky(refX1)
...
matTAux = horaini #Está sobrescrevendo os valores de matTAux
TAux = TX1 #Esta variável está sendo criada agora


##-----Compares if first timestep is after 3 hours of sunrise-----## 
#Posteiormente deve ter um arquivo de horário de nascer e por do sol pra cada pixel da imagem
deltaT = TX1 - 8.
...
indM1 = ((matT1 > horaini) & (matT1 < horafin) & (~np.isnan(dataX1)))
...
for i in range(1,ttfiles,1):
    ...
    dataXI= LibSUN_v1.clearSky(refXI)
...
##-------Trapezoid method------##
        ...
        indMI = ((matTI > matTAux) & (matTI < horafin) & (~np.isnan(dataXI)) & (~np.isnan(dataAux)))
...
##------Replace Matrix Aux to Matrix nº I ---------------##
        ...
        indRep = ((matTI > horaini) & (matTI < horafin) & (~np.isnan(dataXI)))
...
#for i in range(10):
#    data = RegularFunc_v1.getCompleted(data, dataAux)
...
RegularFunc_v1.doDataPlot(xx, yy, matSunTime, beginLatReg, endLatReg,beginLonReg, endLonReg, outputIMG, titleIMG)
print('Created image!')

RegularFunc_v1.createBin(matSunTime, pathBin)
