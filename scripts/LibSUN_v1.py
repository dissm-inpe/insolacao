...
def clearSky(ref):
    ...
    cloud = np.full(ref.shape, np.nan)

    valid = ref > 0

    cloud[valid] = (ref[valid] - Rmin)/(Rmax - Rmin)

    cloud[valid] = np.clip(cloud[valid], 0., 1.)
